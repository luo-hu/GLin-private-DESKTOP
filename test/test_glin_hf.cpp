...
int main() {
 ...
 std::cout << "测试数据生成完成，共" << geoms.size() << "个对象" << std::endl;  // 日志：确认数据生成成功
    ...
    glin_original.glin_bulk_load(geoms, 10000, "zorder", 0, 0, 5, 5, pieces);
    glin_hf.glin_bulk_load(geoms, 10000, "zorder", 0, 0, 5, 5, pieces);  // 会初始化过滤器
    ...
    res_original.clear();
    total_filter_original = 0;
    glin_original.glin_find(query, "zorder", 0, 0, 5, 5, pieces, res_original, total_filter_original);
    std::cout << "原始GLIN查询完成，结果数：" << res_original.size() << std::endl;  // 日志：标记原始GLIN查询完成

    res_hf.clear();
    total_filter_hf = 0;
    glin_hf.glin_find(query, "zorder", 0, 0, 5, 5, pieces, res_hf, total_filter_hf);
