...
TEST_SUITE("GLIN bulk load and find") {
    TEST_CASE("test bulk load and find function non-random") {
        ...
        // GEOS 库中，所有几何对象（如点、线、多边形）都必须通过 GeometryFactory 创建，而工厂需要依赖 PrecisionModel 控制坐标精度。
        // 1. 初始化坐标精度模型（PrecisionModel），PrecisionModel：定义几何坐标的精度（如是否四舍五入到整数、保留几位小数）。这里用默认构造（new geos::geom::PrecisionModel()），表示使用 “浮点精度”（不主动截断或四舍五入坐标）。
        std::unique_ptr<geos::geom::PrecisionModel> pm(new geos::geom::PrecisionModel());
        // 2. 初始化几何对象工厂（GeometryFactory）几何对象的 “工厂类”，负责统一创建和管理几何对象，确保所有对象的精度模型、空间参考一致。第一个参数 pm.get()：绑定前面创建的精度模型；第二个参数 -1：表示 “无空间参考系统（SRID）”（适合测试场景，无需真实地理坐标）。
        geos::geom::GeometryFactory::Ptr global_factory = geos::geom::GeometryFactory::create(pm.get(), -1);;
        //
        //
        // 1. 创建坐标序列容器（存储多边形的顶点坐标）
        geos::geom::CoordinateArraySequence *cl3 = new CoordinateArraySequence();
        // 2. 添加多边形的顶点坐标（按顺时针/逆时针顺序，最后一个点需与第一个点重合，确保闭合）
        ...
        for (int i = 0; i < num_of_poly; i++) {
            ...
            geos::geom::LinearRing *lr6 = global_factory->createLinearRing(cl6);
            geos::geom::Polygon *poly6 = global_factory->createPolygon(lr6, NULL);
            poly_vec.push_back(poly6);//通过循环创建 100 个不同的多边形，存入poy_vec
        }
        //poly_vec的用途是：后续通过 GLIN 的glin_bulk_load接口批量加载到索引中，再用前面创建的query多边形做空间查询，验证 GLIN 索引是否能正确找到 “与query有空间关系的多边形”。
        // count intersection and compare with the find result. 计算查询窗口与多边形的交集，并与查询结果作比较
        double count = 0;
        ...
        //load into index 初始化GLIN索引并批量加载
        index.glin_bulk_load(poly_vec, error_bound, "z", -10.0, -10.0, 5.0, 5.0, pieces);
        // Iterator from beginning to end 验证加载后的数据量是否正确
        int num_keys = 0;
        for (auto it = index.begin(); it != index.end(); ++it) {
            num_keys++;
            // std::cout<<"key at this iterator" <<it.key() << endl;
        }
        CHECK_EQ(100, num_keys);//断言加载的数据量为100

        index.glin_find(query, "z", -10.0, -10.0, 3.0, 3.0, pieces, find_result, count_filter);
        CHECK(find_result.size() == count); //断言查询结果等于交集的数量
        //这部分代码创建了一个不规则的四边形多边形（query），它的角色是 “空间查询窗口”—— 后续会用这个多边形去 GLIN 索引中查询 “与它有空间关系的测试多边形”（如 “包含”Contains、“相交”Intersects）。

        //        CHECK(count > 0);

        /*这段代码是 GLIN 索引测试 / 示例中的 “数据准备阶段”，完整流程通常是：
        用这段代码创建 query（查询窗口）和 poly_vec（100 个测试多边形）；
        调用 index.glin_bulk_load(poly_vec, ...) 将 100 个多边形加载到 GLIN 索引（加载时会自动提取每个多边形的 MBR，转换为 Z 地址区间）；
        调用 index.glin_find(query, ..., results, ...) 执行空间查询（如查找被query包含的多边形）；
        验证 results 中的多边形数量和内容是否符合预期（如 “应该有 20 个多边形被query包含”）。*/
    }
 ...
 */
         ...
         * Rtree running
         */
        geos::index::strtree::STRtree rtree(1000);
        for (int i = 0; i < num_of_poly; i++) {
            //rtree.insert(env,poly_vec[i]);
            // 获取多边形的边界框
            const geos::geom::Envelope* env = poly_vec[i]->getEnvelopeInternal();
            // 插入边界框和几何对象（poly_vec[i]作为用户数据）
            rtree.insert(env, poly_vec[i]);
        }
