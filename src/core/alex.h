 ...
 * - Alex()
 * - void load(V values[], int num_keys)
...
namespace alex {
   ...
   * (base_cost). */
    ...
    public:
        ...
        // recommend directly using load() instead.
...
// node's parent.
    ...
    public:
        ...
        int erase_geo(const T &key, geos::geom::Geometry* geom) {
            data_node_type *leaf = get_leaf(key);
            int num_erased = leaf->erase_mbr(key,geom);
            stats_.num_keys -= num_erased;
            if (leaf->num_keys_ == 0) {
                merge(leaf, key);
            }
    ...
    public:
        ...
        public:
            ...
            // bitmap position
            int num_visited_leaf = 0;
            int num_loaded_leaf = 0;


            geos::geom::Envelope mbr_cumulated = geos::geom::Envelope();
            long double cumulated_line_start = std::numeric_limits<double>::max();
            long double cumulated_line_end = std::numeric_limits<double>::min();
            ...
            Iterator(const Iterator &other)
                    : cur_leaf_(other.cur_leaf_),
                      ...
                      num_visited_leaf(other.num_visited_leaf),
                      num_loaded_leaf(other.num_loaded_leaf),
                      cumulated_line_start(other.cumulated_line_start),
                      cumulated_line_end(other.cumulated_line_end),
                      mbr_cumulated(other.mbr_cumulated){}
            ...
            Iterator &it_update_lineseg(geos::geom::LineSegment perpenline) {
                advance_update_lineseg(perpenline);
                return *this;
            }

            Iterator &it_update_mbr() {
                advance_update_mbr();
                return *this;
            }
            // the perpen line shoule be the perpendicular line of original projection line
            Iterator &it_check_lineseg(long double query_start, long double query_end, typename alex::Alex<T, P>::Iterator it_end) {
                advance_check_lineseg(query_start,query_end, it_end);
                return *this;
            }
            Iterator &it_check_mbr(geos::geom::Envelope* query_window, double max_end) {
                advance_check_mbr(query_window,max_end);
                return *this;
            }
    ...
    V& operator*() const { return cur_leaf_->data_slots_[cur_idx_]; }
        ...
        private:
            void initialize() {
                ...
                cumulated_line_start = std::numeric_limits<double>::max();
                cumulated_line_end = std::numeric_limits<double>::min();
            ...
            forceinline void advance_update_lineseg(geos::geom::LineSegment penper_line) {
                while (cur_bitmap_data_ == 0) {
                    cur_bitmap_idx_++;
                    if (cur_bitmap_idx_ >= cur_leaf_->bitmap_size_) {
                        // Assign the cumulated Line segments to the leaf node
                        cur_leaf_->line_seg_start = cumulated_line_start;
                        cur_leaf_->line_seg_end = cumulated_line_end;
                        cur_leaf_ = cur_leaf_->next_leaf_;
                        cur_idx_ = 0;
                        if (cur_leaf_ == nullptr ) {
                            return;
                        ...
                        num_visited_leaf++;
                        num_loaded_leaf++;
                        cumulated_line_start = std::numeric_limits<long double>::max();
                        cumulated_line_end = std::numeric_limits<long double> ::min();
                    }
                    cur_bitmap_data_ = cur_leaf_->bitmap_[cur_bitmap_idx_];
                }
                long double start;
                ...
                uint64_t bit = extract_rightmost_one(cur_bitmap_data_);
                 cur_idx_ = get_offset(cur_bitmap_idx_, bit);
                cur_bitmap_data_ = remove_rightmost_one(cur_bitmap_data_);
                // Accumulate a single slot
                 shape_projection((*this).payload(), penper_line, start, end);
                if(start < cumulated_line_start) {
                    cumulated_line_start = start;
                }
                if(end > cumulated_line_end) {
                 cumulated_line_end = end;
                }
            }

            forceinline void advance_update_mbr() {
                while (cur_bitmap_data_ == 0) {
                    cur_bitmap_idx_++;
                    if (cur_bitmap_idx_ >= cur_leaf_->bitmap_size_) {
                        // Some nodes could be real nodes but have 0 records, then the MBR will be null. So skip it.
                        if (!this->mbr_cumulated.isNull()) {
                            // Assign the cumulated MBR to the leaf node
                            double minX = this->mbr_cumulated.getMinX();
                            double minY = this->mbr_cumulated.getMinY();
                            double maxX = this->mbr_cumulated.getMaxX();
                            double maxY = this->mbr_cumulated.getMaxY();
                            cur_leaf_->mbr.init(minX, maxX, minY, maxY);
                            mbr_cumulated.setToNull();
                        }
                        cur_leaf_ = cur_leaf_->next_leaf_;
                        cur_idx_ = 0;
                        if (cur_leaf_ == nullptr ) {
                ...
                cur_bitmap_data_ = remove_rightmost_one(cur_bitmap_data_);
                // Accumulate a single slot
                mbr_cumulated.expandToInclude((*this).payload()->getEnvelopeInternal());

            }

            forceinline void
            advance_check_lineseg(long double query_start, long double query_end, typename alex::Alex<T, P>::Iterator it_end) {
                while (cur_bitmap_data_ == 0) {
                    cur_bitmap_idx_++;
                        if (cur_bitmap_idx_ >= cur_leaf_->bitmap_size_) {
                            ...
                            while (cur_leaf_ != it_end.cur_leaf_ && (query_start > cur_leaf_->line_seg_end ||query_end < cur_leaf_->line_seg_start)){
                                cur_leaf_ = cur_leaf_->next_leaf_;
                                num_visited_leaf++;
                            }
                            cur_idx_ = 0;
            ...
            forceinline void advance_check_mbr(geos::geom::Envelope* query_window,double max_end) {
                while (cur_bitmap_data_ == 0) {
                    cur_bitmap_idx_++;
                    if (cur_bitmap_idx_ >= cur_leaf_->bitmap_size_) {
                        // Move to the next leaf
