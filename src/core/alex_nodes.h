...
* - Iterator
* - Cost model
* - Bulk loading and model building (e.g., load, bulk_load_from_existing)
...
*/
    ...
    public:
        ...
        geos::geom::Envelope mbr = geos::geom::Envelope();// the minimum bounding rectangle of this node
        double line_seg_start = 0.0;
        double line_seg_end = 0.0;
        ...
        static constexpr int kDefaultMaxDataNodeBytes_ =
                1 << 16;  // by default, maximum data node size is 16MB
        int max_slots_ =
...
//    delete mbr;
        ...
        AlexDataNode(const self_type &other)
                : AlexNode<T, P>(other),
                  ...
                  mbr(other.mbr),
                  line_seg_start(other.line_seg_start),
                  line_seg_end(other.line_seg_end),
        ...
        static void build_model(const V *values, int num_keys, LinearModel <T> *model,
            ...
            LinearModelBuilder<T> builder(model);
            for (int i = 0; i < num_keys; i++) {
//                std::cout<< "first in value" <<values[i].first << "second in value" << values[i].second->toString() << std::endl;
        ...
        int erase_mbr(const T &key, geos::geom::Geometry* geom) {
            int pos = upper_bound(key);
            ...
            pos--;
            int pos_init = pos; // Record the init position
            int num_real_payload = 0;
            while (pos >= 0 && key_equal(ALEX_DATA_NODE_KEY_AT(pos), key)) {
                if(check_exists(pos)) {
                    num_real_payload ++;
                    if (ALEX_DATA_NODE_PAYLOAD_AT(pos) == geom) {
                        num_erased++;
                        unset_bit(pos);
                    }
                }
            ...
            // Clean up the gaps when no real payloads left in this key region
            // Note: When duplicate keys exist, this function actually delays the gap cleaning until all duplicate keys
            // have been deleted from this key region, instead of cleaning up immediately. This will break the default
            // property of Alex structure: the last key in this key region should not be a gap. This func
            // alex.validate_structure(true, false) will throw warnings but it does not affect the correctness of Glin
            if ((num_real_payload - num_erased) == 0) {
                for (int i = pos_init; i > pos ; --i) {
                    ALEX_DATA_NODE_KEY_AT(i) = next_key;
                }
            }
